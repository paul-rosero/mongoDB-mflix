...
;(async () => {
  try {
    const host = "mongodb+srv://m220student:m220password@mflix.kh6ay.mongodb.net/test"
    console.log(host)
    ...
    // over the wire!
    const predicate = { lastupdated: { $exists: true, $type: "string" } }     //added whats inside both {} for both predicate and projection const.
    ...
    const { modifiedCount } = await mflix.collection("movies").bulkWrite(moviesToMigrate)     //added code after the word await to do bulkWrite.
